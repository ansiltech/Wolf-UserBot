...
@wolfub.tgbot.on(CallbackQuery(data=re.compile(r"^age_verification_true")))
async def age_verification_true(event: CallbackQuery):
    ...
    try:
        await event.edit(
            ...
            file="https://telegra.ph/file/62ebe06cfa510c1904fe1.jpg",
            buttons=buttons,
        )
...
@wolfub.tgbot.on(CallbackQuery(data=re.compile(r"^age_verification_false")))
...
@wolfub.tgbot.on(CallbackQuery(data=re.compile(r"^chg_of_decision_")))
