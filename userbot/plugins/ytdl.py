...
async def download_audio(event):
    ...
    wolfevent = await edit_or_reply(event, "`Preparing to download...`")
    ...
    for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
        if _path.lower().endswith((".jpg", ".png", ".webp")):
            ...
            _fpath = _path
    if not _fpath:
        return await edit_delete(wolfevent, "__Unable to upload file__")
    await wolfevent.edit(
...
async def download_video(event):
    ...
    wolfevent = await edit_or_reply(event, "`Preparing to download...`, Powered by @ansiltechteam")
    reply_to_id = await reply_id(event)
    ytdl_data = await ytdl_down(wolfevent, video_opts, url)
    if ytdl_down is None:
        return
    f = pathlib.Path(f"{ytdl_data['title']}.mp4".replace("|", "_"))
    wolfthumb = pathlib.Path(f"{ytdl_data['title']}.jpg".replace("|", "_"))
    if not os.path.exists(wolfthumb):
        wolfthumb = pathlib.Path(f"{ytdl_data['title']}.webp".replace("|", "_"))
    if not os.path.exists(wolfthumb):
        wolfthumb = None
    await wolfevent.edit(
        f"`Preparing to upload video:`\
...
async def kakashi(event):
    ...
    if "www.instagram.com" not in link:
        ...
        start = datetime.now()
        catevent = await edit_or_reply(event, "**Downloading.....**,**Powered by @ansiltechteam**")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await wolfevent.edit("**Error:** `unblock` @instasavegrambot `and retry!`")
            return
        await wolfevent.delete()
        wolf = await event.client.send_file(
            event.chat_id,
            video,
        )
        end = datetime.now()
