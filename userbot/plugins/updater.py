...
from userbot import HEROKU_APP, UPSTREAM_REPO_URL, wolfub
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        ...
        LOGS.error(e)
    try:
        add_to_collectionlist("restart_update", [ansil.chat_id, ansil.id])
    except Exception as e:
        LOGS.error(e)
    ups_rem.fetch(ac_br)
    repo.git.reset("--hard", "FETCH_HEAD")
...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n`WOLFUSERBOT is`  **up-to-date**  `with`  "
...
@wolfub.wolf_cmd(
    pattern="goodwolf$",
    command=("goodwolf", plugin_category),
    info={
        "header": "To update to goodwolf( For vEg peeps).",
        "usage": "{tr}goodwolf",
    },
)
async def variable(var):
    "To update to goodwolf."
    ...
    await edit_or_reply(var, "`Changing badwolf to goodwolf wait for 2-3 minutes.`")
    heroku_var["UPSTREAM_REPO"] = "https://github.com/ansiltech/Wolf-UserBot"
