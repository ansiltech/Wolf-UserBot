...
async def _(event):
    ...
    wolfevent = await edit_or_reply(event, "`Promoting...`")
    user, rank = await get_user_from_event(event, wolfevent)
    if not rank:
        rank = "Admin"
    if not user:
        return
    try:
        ...
        return await wolfevent.edit(NO_PERM)
    except Exception as e:
        return await edit_delete(wolfevent, f"__{e}__", time=10)
    await wolfevent.edit("`Promoted Successfully! Now gib Party`")
